...
export interface CanvasKeymapOptions {
  onEnterBelow: (initialContent?: RichTextPayload, isEmptyBlock?: boolean) => void
...
export const CanvasKeymap = Extension.create<CanvasKeymapOptions>({
  ...
  addOptions() {
    return {
      onEnterBelow: (_initialContent?, _isEmptyBlock?) => {},
          ...
          handleKeyDown: (view, event) => {
            ...
            if (event.key === 'Enter' && !event.shiftKey && !isMenuOpen && listItem) {
              ...
              if (isEmptyItem && isLastItem) {
                ...
                // When exiting from empty list item, the newly created block will be empty
                onEnterBelow(undefined, true)
                return true
              }
            ...
            if (event.key === 'Enter' && !event.shiftKey && !isInList && !isMenuOpen) {
              ...
              if (atEnd) {
                ...
                onEnterBelow(undefined, isEmpty)
                return true
              }
              ...
              // This is a split operation, not an empty block creation
              onEnterBelow({ json: afterJson, html: afterHtml }, false)
              return true
            }
